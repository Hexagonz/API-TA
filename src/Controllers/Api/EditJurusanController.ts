...
const router = Router();
class EditJurusanController extends AuthMiddleWare {
  ...
  private async editJurusan(
    ...
    try {
      ...
      if (existingJurusan && existingJurusan.id_jurusan != Number(id)) {
        res.status(404).json({
          status: false,
      ...
      const update = await this.jurusan.update({
        where: {
...
export default EditJurusanController;
